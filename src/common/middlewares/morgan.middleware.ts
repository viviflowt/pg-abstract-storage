...
export class MorganMiddleware implements NestMiddleware {
    public static configure(
        ...
        opts?: morgan.Options<http.IncomingMessage, http.ServerResponse>
    ) {
        this.format = format
        this.options = opts
    }
    ...
    ): morgan.Morgan<http.IncomingMessage, http.ServerResponse> {
    ...
    public use(req: any, res: any, next: any) {
        if (MorganMiddleware.format) {
            morgan(MorganMiddleware.format as any, MorganMiddleware.options)(
                req,
                res,
                next
            )

    ...
    database: {
        ...
        logging: Joi.attempt(
            ...
            Joi.alternatives()
                ...
                .default(['error', 'warn', 'log', 'migration', 'schema']),
        ...
        cache: {
            type: 'database',
            duration: Joi.attempt(
                process.env.DB_CACHE_DURATION,
                Joi.number().default(1000),
                'DB_CACHE_DURATION must be a valid number'
            ),
            tableName: Joi.attempt(
                process.env.DB_CACHE_TABLE_NAME,
                Joi.string().default('typeorm_cache'),
                'DB_CACHE_TABLE_NAME must be a valid string'
            ),
            alwaysEnabled: Joi.attempt(
                process.env.DB_CACHE_ALWAYS_ENABLED,
                Joi.boolean().default(false),
                'DB_CACHE_ALWAYS_ENABLED must be a valid boolean'
            )
        },
